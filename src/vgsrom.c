...
int main(int argc,char* argv[])
{
	...
	if(argc<3) {
		puts("usage: vgsrom input-dir output-rom-file");
		return 1;
	}
...
int makelist(char* dpath)
{
	...
	do {
		if(NULL!=(ent=readdir(dir))) {
			if((strncmp(ent->d_name,"PSLOT",5)==0 && strcmp(&(ent->d_name[8]),".BIN")==0)
			|| (strncmp(ent->d_name,"GSLOT",5)==0 && strcmp(&(ent->d_name[8]),".CHR")==0)
